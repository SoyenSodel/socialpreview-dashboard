...
export const TeamTicketDetailModal = ({ ticket, onClose, onUpdate }: TicketDetailModalProps) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await fetch(getApiUrl(`api/tickets/${ticket.id}/comments`), {
          credentials: 'include',
        });
  ...
  const sendComment = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      await fetch(getApiUrl(`api/tickets/${ticket.id}/comments`), {
        method: 'POST',
        headers: {
  ...
  const updateTicketStatus = async (status: string) => {
    try {
      await fetch(getApiUrl(`api/tickets/${ticket.id}`), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500">
                <span>{t('ticket.createdBy')} {sanitizeText(ticket.user_name)}</span>
        ...
        <div className="p-6 border-t border-zinc-900">
          {isTicketResolved ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 text-center">
              <p className="text-green-400 font-medium">{t('ticket.resolvedCannotReply')}</p>
            </div>
          ) : (
