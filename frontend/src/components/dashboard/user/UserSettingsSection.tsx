...
export const UserSettingsSection = () => {
        ...
        xhr.addEventListener('load', async () => {
          ...
          if (xhr.status === 200) {
            try {
              ...
              if (data.success) {
                setProfileMessage({ type: 'success', text: t('userDashboard.profileUpdated') });
                setSelectedFile(null);
                await refreshUser();
                resolve();
              } else {
                setProfileMessage({ type: 'error', text: data.error || t('userDashboard.failedToUpdateProfile') });
                reject(new Error(data.error));
              }
            } catch (error) {
              setProfileMessage({ type: 'error', text: t('error.networkError') });
              reject(error);
            }
          } else {
            setProfileMessage({ type: 'error', text: t('error.networkError') });
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setMessage({ type: 'error', text: t('error.passwordsDoNotMatch') });
      return;
    }
    if (newPassword.length < 8) {
      setMessage({ type: 'error', text: t('error.passwordTooShort') });
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setMessage({ type: 'success', text: t('success.passwordChanged') });
        setCurrentPassword('');
        setNewPassword('');
        setConfirmPassword('');
      } else {
      ...
      setMessage({ type: 'error', text: t('error.networkErrorRetry') });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetup2FA = async () => {
    ...
    try {
      ...
      setTwoFAMessage({ type: 'error', text: t('error.networkErrorRetry') });
    } finally {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-white">{t('userDashboard.settings')}</h2>
      ...
      <div className="bg-zinc-950/80 backdrop-blur-xl border border-zinc-900 rounded-lg overflow-hidden max-w-4xl mx-auto">
        ...
        <div className="p-8">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-xl font-semibold text-white mb-2">{t('userDashboard.editProfile')}</h3>
                <p className="text-sm text-gray-400">Update your personal information and profile picture</p>
              </div>
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">{t('userDashboard.profileName')}</label>
                    <input
                      type="text"
                      value={name}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">{t('userDashboard.profileNickname')}</label>
                    <input
                      type="text"
                      value={nickname}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-3">{t('userDashboard.profilePicture')}</label>
                ...
                <div className="pt-4 border-t border-zinc-800">
                  ...
                  >
                    {profileLoading ? t('userDashboard.updatingProfile') : t('userDashboard.updateProfile')}
                  </button>
                </div>
              </form>
            </div>
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
              { }
              <div>
                <h3 className="text-xl font-semibold text-white mb-2">{t('userDashboard.changePassword')}</h3>
              ...
              <form onSubmit={handleChangePassword} className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">{t('userDashboard.currentPassword')}</label>
                  <input
                    type="password"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">{t('userDashboard.newPassword')}</label>
                  <input
                    type="password"
                    value={newPassword}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">{t('userDashboard.confirmNewPassword')}</label>
                  <input
                    type="password"
                ...
                <div className="pt-4 border-t border-zinc-800">
                  <button
                    ...
                    className="px-6 py-3 bg-white text-black rounded-lg hover:bg-gray-100 transition-colors font-medium disabled:opacity-50"
                  >
                    {loading ? t('userDashboard.changingPassword') : t('userDashboard.changePassword')}
                  </button>
                </div>
              </form>
